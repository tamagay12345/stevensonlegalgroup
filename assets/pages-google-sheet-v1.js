...
const CONSULTATION_WEB_APP_URL = "https://script.google.com/macros/s/AKfycbxYNAsklGD914bB3ORREzabeoUJTLoexXWuKJ8XldCV72gCydYhD09NKsK_yYcPG-BmVQ/exec";
...
function buildConsultationPayload(form) {
    ...
    return {
        ...
        contactMethod: readValue("contactMethod"),
        service: readValue("service"),
        location: readValue("location"),
        preferredDate: readValue("preferredDate"),
        preferredTime: readValue("preferredTime"),
        opposingParty: readValue("opposingParty"),
        deadline: readValue("deadline"),
...
async function saveConsultationRequest(payload) {
    await fetch(CONSULTATION_WEB_APP_URL, {
        method: "POST",
        mode: "no-cors",
        cache: "no-store",
        keepalive: true,
    ...
    modal.innerHTML = `
        ...
        >
            ...
            <p id="consultation-success-description">
                Your completed details have been cleared from the form. You can
                close this message or go back to enter a new consultation request.
            </p>
...
function initializeConsultationForm() {
    ...
    consultationForm.addEventListener("submit", async (event) => {
        ...
        let formStatus = consultationForm.querySelector("#form-status");

        if (!formStatus) {
            formStatus = document.createElement("span");
        ...
        const payload = buildConsultationPayload(consultationForm);
        ...
        formStatus.textContent = "Submitting your consultation request...";
        ...
        try {
            ...
            formStatus.textContent =
                "The request could not be submitted. Please check your connection and try again.";
        } finally {
            if (submitButton) {
